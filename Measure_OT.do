...
preserve
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_OT"
keep if product=="资本品"
save Match`i'_Import_OT_Capital.dta,replace
restore
keep if product=="中间品"
...
cd "F:\Measure\DVAR_Input\DVAR_OT"
save Imp_Inter_Ratio_OT_`i'.dta,replace
restore

merge m:1 hs6 using Imp_Inter_Ratio_OT_`i'.dta
...
drop _m
save Match`i'_Import_id_OT.dta,replace
cd "F:\Measure\OriginalData"
use Match`i'_Export.dta,clear
keep if shipment=="一般贸易" | shipment=="来料加工装配贸易" | shipment=="进料加工贸易"
cd "F:\Measure\DVAR_Input\DVAR_OT"
merge m:1 id_new using Match`i'_Import_id_OT.dta  //与加工贸易企业匹配，得到加工贸易方式出口的企业
...
save Exp_Inter_Ratio_OT_`i'.dta,replace
restore
merge m:1 hs6 using Exp_Inter_Ratio_OT_`i'.dta
drop _m
...
duplicates drop id_new hs6 ,force
bysort id_new: egen value_exp_OT_real_id= sum(value_exp_id_hs)
duplicates drop id_new,force     //得到每个id_new的实际出口总额
save Match`i'_Import_Export_id_OT.dta ,replace   //实际进口和实际出口
*资本品
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_OT"
use Match`i'_Import_OT_Capital.dta,clear
...
save Imp_Inter_Ratio_OT_Cap_`i'.dta,replace
restore

keep if Intermediary==0
merge m:1 hs6 using Imp_Inter_Ratio_OT_Cap_`i'.dta
...
bysort id_new: egen value_imp_ot_real_id_cap= sum(value_imp_id_hs_cap) 
duplicates drop id_new value_imp_ot_real_id_cap,force
capture drop _m
save Match`i'_OT_Inter_Cap.dta,replace   
label data 考虑了贸易中间商的资本品进口额

}  
else{
bysort id_new:egen value_imp_ot_real_id_cap=sum(value) 
duplicates drop id_new value_imp_ot_real_id_cap,force
capture drop _m
save Match`i'_OT_Inter_Cap.dta,replace   
label data 考虑了贸易中间商的资本品进口额
...
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_OT"
use Match`i'_OT_Inter_Cap.dta,clear
rename value_imp_ot_real_id_cap id`i'_cap
keep id_new id`i'_cap
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_OT\OT_id_cap"
