...
preserve
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT"
keep if product=="资本品"
...
cd "F:\Measure\DVAR_Input\DVAR_PT"
save Imp_Inter_Ratio_PT_`i'.dta,replace
restore

merge m:1 hs6 using Imp_Inter_Ratio_PT_`i'.dta
...
drop _m
save Match`i'_Import_id_PT.dta,replace
cd "F:\Measure\OriginalData"
use Match`i'_Export.dta,clear
keep if shipment=="一般贸易" | shipment=="来料加工装配贸易" | shipment=="进料加工贸易"
cd "F:\Measure\DVAR_Input\DVAR_PT"
merge m:1 id_new using Match`i'_Import_id_PT.dta  //与加工贸易企业匹配，得到加工贸易方式出口的企业
...
save Exp_Inter_Ratio_PT_`i'.dta,replace
restore
merge m:1 hs6 using Exp_Inter_Ratio_PT_`i'.dta
drop _m
...
duplicates drop id_new hs6 ,force
bysort id_new: egen value_exp_pt_real_id= sum(value_exp_id_hs)
duplicates drop id_new,force     //得到每个id_new的实际出口总额
save Match`i'_Import_Export_id_PT.dta ,replace   //实际进口和实际出口
*资本品
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT"
use Match`i'_Import_PT_Capital.dta,clear
...
save Imp_Inter_Ratio_PT_Cap_`i'.dta,replace
restore

keep if Intermediary==0
merge m:1 hs6 using Imp_Inter_Ratio_PT_Cap_`i'.dta
...
bysort id_new: egen value_imp_pt_real_id_cap= sum(value_imp_id_hs_cap) 
duplicates drop id_new value_imp_pt_real_id_cap,force
capture drop _m
save Match`i'_PT_Inter_Cap.dta,replace   
label data 考虑了贸易中间商的资本品进口额

}  
else{
bysort id_new:egen value_imp_pt_real_id_cap=sum(value) 
duplicates drop id_new value_imp_pt_real_id_cap,force
capture drop _m
save Match`i'_PT_Inter_Cap.dta,replace   
label data 考虑了贸易中间商的资本品进口额
...
**将来料加工装配进口的设备的资本品加总
**%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
cd "F:\Measure\Measure_DVAR\DVAR"
forvalues i=2000(1)2006{
use Match`i'_Import_PT_Capital.dta,clear
/*capture drop mix     //来料加工装配进口的设备
bysort id_new:egen value_cap_PT=sum(value)
...
forvalues i=2000(1)2006{
/*use Match`i'_Import_PT_Capital.dta,clear
keep id_new value_cap_PT
save Match`i'_Import_PT_Capital.dta,replace*/
use Match`i'_Import_Export_id_PT.dta,clear
merge 1:1 id_new using Match`i'_Import_PT_Capital.dta
drop if _m==2
drop _m
replace value_cap_PT=0 if value_cap_PT==.
save Match`i'_cap_real_PT.dta,replace
}
...
**===============================================================
**将考虑贸易中间商进口的实际资本品与来料加工装配进口的设备进行加总
**===============================================================
forvalues i=2000(1)2006{
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT"
use Match`i'_PT_Inter_Cap.dta,clear
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT\Old_version"
merge 1:1 id_new using Match`i'_Import_PT_Capital.dta
keep id_new value_imp_pt_real_id_cap value_cap_PT _merge
replace value_imp_pt_real_id_cap=0 if value_imp_pt_real_id_cap ==.
replace value_cap_PT =0 if value_cap_PT==. 
gen id`i'_cap=value_imp_pt_real_id_cap+value_cap_PT
keep id_new id`i'_cap
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT\PT_id_cap"
...
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT"
use Match`i'_PT_Inter_Cap.dta,clear
rename value_imp_pt_real_id_cap id`i'_cap
keep id_new id`i'_cap
cd "F:\Measure\DVAR_AccumulatedDepreciation\DVAR_PT\PT_id_cap"
save id`i'_cap.dta,replace
